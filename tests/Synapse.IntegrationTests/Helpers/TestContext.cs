...
namespace Synapse.IntegrationTests.Helpers;
...
public static class TestContext
{
    ...
    /// folder lives outside the repo — e.g. when _LOCAL_BUILD_ROOT
    /// redirects output to %LOCALAPPDATA%\-dev\build\... on a
    /// network-share repo, where no parent of the bin dir contains .sln.
    /// </summary>
    ...
    private static string FindSolutionDir([CallerFilePath] string callerPath = "")
    {
        ...
        return dir ?? throw new InvalidOperationException(
            "Could not find .sln walking up from " + callerPath);
    }
}
