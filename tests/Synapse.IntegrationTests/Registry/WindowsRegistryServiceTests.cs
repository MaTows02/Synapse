...
namespace Synapse.IntegrationTests.Registry;
...
public class WindowsRegistryServiceTests : IDisposable
{
    ...
    public WindowsRegistryServiceTests()
    {
        ...
        _testKeyRoot = $@"Software\IntegrationTests\{guid}";
    ...
    public void SetValue_String_ReadBackMatches()
    {
        ...
        var testValue = "Hello,  Integration Tests!";
    ...
    public void Dispose()
    {
        ...
        // Also clean up parent "IntegrationTests" if empty
        try
        {
            using var parent = Microsoft.Win32.Registry.CurrentUser.OpenSubKey(
                @"Software\IntegrationTests", writable: false);
            if (parent?.SubKeyCount == 0)
            {
                Microsoft.Win32.Registry.CurrentUser.DeleteSubKeyTree(
                    @"Software\IntegrationTests", throwOnMissingSubKey: false);
            }
        }
        catch
        {
