...
using Synapse.UI.Features.Common.ViewModels;
using Xunit;

namespace Synapse.UI.Tests.ViewModels;
...
public class MoreMenuViewModelTests
{
    ...
    public void Constructor_InitializesVersionInfo_WithVersionFromService()
    {
        ...
        vm.VersionInfo.Should().Be(" v25.06.01");
    }

    [Fact]
    public void Constructor_WhenVersionServiceThrows_DefaultsTo()
    {
        _mockVersionService
            .Setup(v => v.GetCurrentVersion())
    ...
    public void Constructor_SubscribesToLanguageChanged()
    {
        ...
        changedProperties.Should().Contain(nameof(vm.MenuLogs));
        changedProperties.Should().Contain(nameof(vm.MenuChangeHistory));
        changedProperties.Should().Contain(nameof(vm.MenuScripts));
        changedProperties.Should().Contain(nameof(vm.MenuClose));
    }
    ...
    [Fact]
    public void MenuLogs_ReturnsLocalizedString()
    {
        _mockLocalization
            .Setup(l => l.GetString("Menu_Logs"))
            .Returns("Logs");

        var vm = CreateViewModel();

        vm.MenuLogs.Should().Be("Logs");
    }

    [Fact]
    public void MenuScripts_ReturnsLocalizedString()
    {
        _mockLocalization
            .Setup(l => l.GetString("Menu_Scripts"))
            .Returns("Scripts");

        var vm = CreateViewModel();

        vm.MenuScripts.Should().Be("Scripts");
    }

    [Fact]
    public void MenuClose_ReturnsLocalizedString()
    {
        _mockLocalization
            .Setup(l => l.GetString("Menu_Close"))
            .Returns("Exit");

        var vm = CreateViewModel();

        vm.MenuClose.Should().Be("Exit");
    }
    ...
    {
        _mockChangeHistory.Setup(h => h.GetFilePath()).Returns(@"C:\ProgramData\\ChangeHistory.txt");
        ...
        _mockProcessExecutor.Verify(
            p => p.ShellExecuteAsync(@"C:\ProgramData\\ChangeHistory.txt", null, false, It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    public void VersionInfo_Set_RaisesPropertyChanged()
    {
        ...
        vm.VersionInfo = " v99.0.0";

        raised.Should().BeTrue();
        vm.VersionInfo.Should().Be(" v99.0.0");
    }
