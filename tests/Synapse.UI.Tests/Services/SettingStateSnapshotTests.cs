...
public class SettingStateSnapshotTests
{
    ...
    /// shipped . config presets (_Recommended_Config. and
    /// _Default_Config_Windows11_25H2.). Those configs were the only reliable
    ...
    private static object? ResolveRecommended(SettingDefinition s)
    {
        if (s.InputType == InputType.Selection)
        {
            ...
            // the shipped . config SelectedIndex values correctly into the
        ...
        // pre-refactor from the shipped _Recommended_Config) only captured *positive*
