...
public class SettingCatalogValidatorTests
{
    ...
    // The Preference pill says "this is a matter of taste"; the Recommended pill says "but 
    // suggests this option." Both pills display independently. A subjective setting MAY carry a
    //  recommendation — there is no constraint preventing it.
    // IsSubjectivePreference applies to any InputType (Toggle, CheckBox, Selection, NumericRange, ...),
    // not just Selection. For Toggles/CheckBoxes it signals "user choice —  makes no
    ...
    ///  recommendation — either via the toggle-level <see cref="SettingDefinition.RecommendedToggleState"/>
    ...
    /// flag explicitly means "user choice, no  recommendation."
    /// </summary>
    [Theory]
    [MemberData(nameof(AllSettings))]
    public void Toggle_HasRecommendation(string id, SettingDefinition s)
    {
        ...
        (hasToggleLevelFlag || hasPerKeyValue).Should().BeTrue(
            $"{id} is a Toggle/CheckBox and must declare a  recommendation — set " +
