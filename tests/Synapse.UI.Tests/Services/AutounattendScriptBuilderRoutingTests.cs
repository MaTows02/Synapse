...
/// AutounattendScriptBuilder. Directly exercises BuildmentsScriptAsync and inspects
...
public class AutounattendScriptBuilderRoutingTests
{
    ...
    public async Task PowerShellScript_MarkedUser_LandsInUserPassOnly()
    {
        ...
        var script = await builder.BuildmentsScriptAsync(
    ...
    public async Task RegContents_HkcuMentionedInComment_StillRoutesToSystemPass()
    {
        ...
        var def = new SettingDefinition
        {
            ...
            RegistrySettings = new List<RegistrySetting>
            {
                new()
                {
                    KeyPath = @"HKEY_CLASSES_ROOT\.Test",
                    ValueName = "",
                    EnabledValue = ["test"],
                    DisabledValue = [null],
                    DefaultValue = null,
        ...
        var (system, user) = SplitPasses(script);
        system.Should().Contain(@"HKEY_CLASSES_ROOT\.Test").And.Contain("reg import");
        user.Should().NotContain(@"HKEY_CLASSES_ROOT\.Test");
    }
    ...
    public async Task RegContents_MixedHiveBlock_ThrowsOnBuild()
    {
        var def = new SettingDefinition
        {
            ...
            RegistrySettings = new List<RegistrySetting>
            {
                new()
                {
                    KeyPath = @"HKEY_LOCAL_MACHINE\SOFTWARE\\MixedTest",
                    ValueName = "Flag",
                    EnabledValue = [1],
                    DisabledValue = [0],
                    RecommendedValue = 1,
        ...
        var act = async () => await builder.BuildmentsScriptAsync(
            ConfigWithOptimize("test-feature", item),
            SingleSetting("test-feature", def));
    ...
    public async Task HklmRegistryToggle_LandsInSystemPassOnly()
    {
        ...
        var (system, user) = SplitPasses(script);
        system.Should().Contain(@"HKLM:\SOFTWARE\\TestKey");
        user.Should().NotContain(@"HKLM:\SOFTWARE\\TestKey");
    }
