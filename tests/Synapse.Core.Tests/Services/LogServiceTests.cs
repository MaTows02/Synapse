...
namespace Synapse.Core.Tests.Services;
...
public class LogServiceTests
{
    ...
    [Fact]
    public void GetLogPath_ContainsDirectory()
    {
        using var service = new LogService();
    ...
    public void Log_WithDifferentLevels_DoesNotThrow()
    {
        ...
        var action = () =>
        {
            service.Log(Synapse.Core.Features.Common.Enums.LogLevel.Info, "info message");
            service.Log(Synapse.Core.Features.Common.Enums.LogLevel.Warning, "warning message");
            service.Log(Synapse.Core.Features.Common.Enums.LogLevel.Error, "error message");
            service.Log(Synapse.Core.Features.Common.Enums.LogLevel.Debug, "debug message");
            service.Log(Synapse.Core.Features.Common.Enums.LogLevel.Success, "success message");
        };
    ...
    public void Log_WithException_DoesNotThrow()
    {
        ...
        var action = () => service.Log(
            Synapse.Core.Features.Common.Enums.LogLevel.Error, "error", ex);
    ...
    public void StartLog_WithSystemInfoProvider_WritesAllDiagnosticFields()
    {
        ...
        logContent.Should().Contain(" 26.02.20 Log Started");
    ...
    public void StartLog_WithoutSystemInfoProvider_WritesFallbackMessage()
    {
        ...
        var logContent = File.ReadAllText(logPath);
        logContent.Should().Contain(" Log Started");
    ...
    public void CleanupOldLogs_DeletesFilesOlderThanMaxAge()
    {
        ...
        try
        {
            ...
            var oldFile = Path.Combine(tempDir, "_Log_20200101_000000.log");
            File.WriteAllText(oldFile, "old");
            File.SetCreationTimeUtc(oldFile, DateTime.UtcNow.AddDays(-60));

            // Create a "recent" log file
            var recentFile = Path.Combine(tempDir, "_Log_20260226_120000.log");
    ...
    public void CleanupOldLogs_CapsToMaxFiles()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                var file = Path.Combine(tempDir, $"_Log_20260226_{i:D6}.log");
            ...
            var remaining = Directory.GetFiles(tempDir, "_Log_*.log");
