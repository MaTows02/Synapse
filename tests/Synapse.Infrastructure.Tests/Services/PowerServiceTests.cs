...
using Synapse.Infrastructure.Features.Optimize.Services;
using Xunit;
...
public class PowerServiceTests
{
    ...
    {
        // Arrange
        var expectedPlan = new Synapse.Core.Features.Optimize.Models.PowerPlan
        {
            Name = "Balanced",
    ...
    {
        // Arrange
        var plans = new List<Synapse.Core.Features.Optimize.Models.PowerPlan>
        {
    ...
    public async Task DiscoverSpecialSettingsAsync_WithPowerPlanSetting_ReturnsActivePlanInfo()
    {
        ...
        var activePlan = new Synapse.Core.Features.Optimize.Models.PowerPlan
        {
            Name = "Balanced",
            Guid = "381b4222-f694-41f0-9685-ff5bb260df2e"
        };
    ...
    public async Task TryApplySpecialSettingAsync_CorruptPlan_DeletesGhostAndAttemptsImport()
    {
        ...
        _logService.Verify(
            l => l.Log(LogLevel.Warning, It.Is<string>(s => s.Contains("corrupt  plan"))),
            Times.Once);

        // Name should be set on the actual GUID powercfg assigned, not the requested one
        _powerSchemeOperations.Verify(
            s => s.WriteFriendlyName(Guid.Parse(assignedGuid), "Synapse Power Plan"),
            Times.Once);
    }
    ...
    public async Task TryApplySpecialSettingAsync_ConfigImportSuppliesPowerValues_SkipsRecommendedReapply()
    {
        ...
        var (setting, value, apply) = ArrangePlanImport();
