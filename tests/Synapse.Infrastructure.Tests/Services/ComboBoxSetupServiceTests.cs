...
public class ComboBoxSetupServiceTests
{
        ...
        Dictionary<int, Dictionary<string, object?>>? valueMappings = null)
    {
        ...
        if (displayNames != null || valueMappings != null)
        {
            ...
            var options = new List<Synapse.Core.Features.Common.Models.ComboBoxOption>(names.Length);
            for (int i = 0; i < names.Length; i++)
            {
                options.Add(new Synapse.Core.Features.Common.Models.ComboBoxOption
                {
                    DisplayName = names[i],
    ...
    public async Task SetupComboBoxOptionsAsync_PowerPlanSetting_DelegatesToPowerPlanService()
    {
        ...
        expectedResult.Options.Add(new Synapse.Core.Features.Common.Interfaces.ComboBoxDisplayOption("Balanced", 0));
        expectedResult.Options.Add(new Synapse.Core.Features.Common.Interfaces.ComboBoxDisplayOption("High Performance", 1));
        expectedResult.Options.Add(new Synapse.Core.Features.Common.Interfaces.ComboBoxDisplayOption("Ultimate", 2));
