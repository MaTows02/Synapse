# PowerShell script that downloads the latest beta (pre-release) version of Synapse and runs the Installer
...
function Get-FileFromWeb {
    ...
    try {
        ...
        do {
            ...
            $total += $count
            if ($fullSize -gt 0) { Show-Progress -TotalValue $fullSize -CurrentValue $total -ProgressText " Downloading Synapse Beta" }
        } while ($count -gt 0)
    }
    finally {
        $reader.Close()
...
$installerPath = "C:\ProgramData\Synapse\Unattend\SynapseBetaInstaller.exe"
...
try {
    ...
    $asset = $betaRelease.assets | Where-Object { $_.name -eq "Synapse.Installer.Beta.exe" } | Select-Object -First 1
    ...
    Write-Host "       Synapse Beta Installer" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "Downloading Synapse Beta ($installType)..." -ForegroundColor Cyan
    ...
    if ($choice -eq '1') {
        ...
        Write-Host "Installing Synapse Beta..." -ForegroundColor Cyan
        ...
        $appPath = Join-Path $env:ProgramFiles "Synapse\Synapse.exe"
        if (Test-Path $appPath) {
            Write-Host "Launching Synapse..." -ForegroundColor Cyan
            Start-Process -FilePath $appPath
        }
    } else {
        # Portable installation - extract to Desktop
        Write-Host ""
        Write-Host "Extracting Synapse Beta Portable to Desktop..." -ForegroundColor Cyan
        ...
        Write-Host ""
        Write-Host "Synapse Portable has been extracted to your Desktop." -ForegroundColor Cyan
        Write-Host "You can move the Synapse folder to your desired location if needed." -ForegroundColor Cyan
        $appPath = Join-Path ([System.Environment]::GetFolderPath('Desktop')) "Synapse\Synapse.exe"
        if (Test-Path $appPath) {
            Write-Host "Launching Synapse..." -ForegroundColor Cyan
            Start-Process -FilePath $appPath
        }
