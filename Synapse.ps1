# PowerShell script that downloads the latest version of Synapse and runs the Installer
...
function Get-FileFromWeb {
    ...
    try {
        ...
        do {
            ...
            $total += $count
            if ($fullSize -gt 0) { Show-Progress -TotalValue $fullSize -CurrentValue $total -ProgressText " Downloading Synapse" }
        } while ($count -gt 0)
    }
    finally {
        $reader.Close()
...
try {
    ...
    Write-Host ""
    Write-Host "Downloading Synapse ($installType)..." -ForegroundColor Cyan
    ...
    if ($choice -eq '1') {
        ...
        Write-Host "Installing Synapse..." -ForegroundColor Cyan
        ...
        Write-Host ""
        Write-Host "Extracting Synapse Portable to Desktop..." -ForegroundColor Cyan
