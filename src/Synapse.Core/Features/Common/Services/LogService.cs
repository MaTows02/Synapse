...
namespace Synapse.Core.Features.Common.Services;
...
public class LogService : ILogService, IDisposable
{
    ...
    public LogService()
    {
        _logPath = Path.Combine(
            ...
            "Synapse",
            "Logs",
            $"Synapse_Log_{DateTime.Now:yyyyMMdd_HHmmss}.log"
        );
    }
    ...
    public void StartLog()
    {
        try
        {
            ...
            if (_systemInfoProvider != null)
            {
                ...
                LogInformation($"====  {info.AppVersion} Log Started ====");
    ...
    internal static void CleanupOldLogs(string logDirectory, int maxAgeDays = 30, int maxFiles = 50)
    {
        try
        {
            ...
            var logFiles = Directory.GetFiles(logDirectory, "_Log_*.log")
