...
using static Synapse.Infrastructure.Features.AdvancedTools.Helpers.PowerShellScriptUtilities;

namespace Synapse.Infrastructure.Features.AdvancedTools.Helpers;
...
internal class RegistryCommandEmitter
{
    ...
    public void AppendRegContentCommands(StringBuilder sb, SettingDefinition setting, bool? isEnabled, bool isHkcuPass, string indent = "")
    {
        ...
        foreach (var regContent in setting.RegContents!)
        {
            ...
            sb.AppendLine("'@");
            sb.AppendLine($"{indent}    $tempRegFile = Join-Path $env:TEMP \"_{setting.Id}_$((Get-Date).Ticks).reg\"");
