...
namespace Synapse.Infrastructure.Features.SoftwareApps.Services;

public class AppIconResolver : IAppIconResolver
{
    private const string CacheSubDir = @"\IconCache";

    // Cache filenames are <def.Id>.<short-hash>.png. The id makes them readable
    // when poking around %ProgramData%\\IconCache; the 8-char hash of
    ...
    // manually wipe %ProgramData%\\IconCache so cached files re-extract.
    ...
    /// <summary>Production constructor — uses %ProgramData%\\IconCache.</summary>
    ...
    // Icons are app-wide reference data. ProgramData keeps the cache alongside
    // Synapse's other shared state (for example C:\ProgramData\Synapse\Logs)
    // and means a single
        ...
        bool allowRawFallback = true)
    {
        ...
        // %ProgramData%\\Logs instead of silently dropping the
