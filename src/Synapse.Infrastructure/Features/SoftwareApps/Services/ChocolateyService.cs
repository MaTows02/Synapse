...
public class ChocolateyService : IChocolateyService
{
    ...
    public async Task<bool> InstallPackageAsync(string chocoPackageId, string? displayName = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // specifically because  detection saw the app as NOT installed — so this
            ...
            {
                _logService.LogWarning($"Chocolatey reports '{chocoPackageId}' as already-installed but  detection saw it as missing. Clearing stale record and retrying install.");
