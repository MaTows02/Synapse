...
public class DirectDownloadService : IDirectDownloadService
{
    ...
    private static readonly string UserAgent = "-Download-Manager";
    ...
    {
        var tempPath = _fileSystemService.CombinePath(_fileSystemService.GetTempPath(), $"_{item.Id}_{Guid.NewGuid():N}");
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var logPath = _fileSystemService.CombinePath(_fileSystemService.GetTempPath(), $"_MSI_{Guid.NewGuid():N}.log");
            ...
            if (exitCode == 1612)
            {
                ...
                logPath = _fileSystemService.CombinePath(_fileSystemService.GetTempPath(), $"_MSI_{Guid.NewGuid():N}.log");
