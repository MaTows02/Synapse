...
{
    // Dev-only override: when _LOCAL_INSTALLER points at an existing
    ...
    private const string LocalInstallerEnvVar = "SYNAPSE_LOCAL_INSTALLER";
    ...
    public async Task DownloadAndInstallUpdateAsync(CancellationToken cancellationToken = default)
    {
        ...
        string tempPath = _fileSystemService.CombinePath(_fileSystemService.GetTempPath(), ".Installer.exe");
    ...
    public void LaunchInstallerAndRestart()
    {
        ...
        string appExePath = _fileSystemService.CombinePath(appDir, ".exe");
    ...
    /// <summary>
    /// Returns the local installer path if _LOCAL_INSTALLER is set and
    ...
    /// can resolve {app} to C:\Program Files\ for regular installs at a
    /// custom path, or to ~\Desktop\ for portable installs, silently
    /// relocating the install. The installer script (.Installer.iss)
