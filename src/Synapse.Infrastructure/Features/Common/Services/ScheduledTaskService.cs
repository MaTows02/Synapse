...
public class ScheduledTaskService(ILogService logService, IFileSystemService fileSystemService) : IScheduledTaskService
{
    ...
    public async Task<OperationResult> UnregisterScheduledTaskAsync(string taskName)
    {
        return await Task.Run(() =>
        {
            ...
            try
            {
                ...
                {
                    logService.Log(Synapse.Core.Features.Common.Enums.LogLevel.Debug, $"[ScheduledTaskService] Task '{taskName}' not found: {ex.Message}");
                }
    ...
    public async Task<bool> IsTaskRegisteredAsync(string taskName)
    {
        return await Task.Run(() =>
        {
            ...
            {
                logService.Log(Synapse.Core.Features.Common.Enums.LogLevel.Debug, $"[ScheduledTaskService] Task '{taskName}' not registered: {ex.Message}");
                return false;
            }
            finally
            {
    ...
    public async Task<OperationResult> RunScheduledTaskAsync(string taskName)
    {
        return await Task.Run(() =>
        {
            ...
            try
            {
                ...
                if (folder == null)
                {
                    logService.LogError($" task folder not found when trying to run: {taskName}");
                    return OperationResult.Failed(" task folder not found");
                }
    ...
    private async Task<OperationResult> RegisterTaskInternal(string taskName, string? scriptPath, string? username, TaskTriggerType triggerType, string? command = null)
    {
        ...
        {
            taskService = CreateTaskService();
            folder = GetOrCreateFolder(taskService);
    ...
    private dynamic GetOrCreateFolder(dynamic taskService)
    {
        dynamic rootFolder = taskService.GetFolder("\\");
        try
        {
            return rootFolder.GetFolder("Synapse");
        }
        catch (Exception ex)
        {
            logService.Log(Synapse.Core.Features.Common.Enums.LogLevel.Debug, $"[ScheduledTaskService] Synapse folder doesn't exist, creating: {ex.Message}");
            return rootFolder.CreateFolder("Synapse");
        }
        finally
        {
    ...
    private dynamic? GetFolder(dynamic taskService)
    {
        ...
        {
            logService.Log(Synapse.Core.Features.Common.Enums.LogLevel.Debug, $"[ScheduledTaskService] Synapse folder not found: {ex.Message}");
            return null;
        }
        finally
        {
    ...
    private async Task RemoveExistingTask(dynamic folder, string taskName)
    {
        ...
        {
            logService.Log(Synapse.Core.Features.Common.Enums.LogLevel.Debug, $"[ScheduledTaskService] No existing task '{taskName}' to remove: {ex.Message}");
        }
        finally
        {
            ReleaseComObject(existingTask);
    ...
    public async Task<bool?> IsTaskEnabledAsync(string taskPath)
    {
        return await Task.Run(() =>
        {
            ...
            {
                logService.Log(Synapse.Core.Features.Common.Enums.LogLevel.Warning,
