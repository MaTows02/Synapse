...
namespace Synapse.Infrastructure.Features.Common.Utilities;
...
public class PowerShellRunner : IPowerShellRunner
{
        ...
        CancellationToken ct = default)
    {
        ...
        var tempFile = _fileSystemService.CombinePath(_fileSystemService.GetTempPath(), $"_{Guid.NewGuid()}.ps1");
        try
        {
        ...
        CancellationToken ct = default)
    {
        ...
        var tempFile = _fileSystemService.CombinePath(_fileSystemService.GetTempPath(), $"_validate_{Guid.NewGuid():N}.ps1");
        try
        {
...
exit 0";
    ...
    public async Task ValidateXmlSyntaxAsync(
        ...
        CancellationToken ct = default)
    {
        var tempFile = _fileSystemService.CombinePath(_fileSystemService.GetTempPath(), $"_validate_{Guid.NewGuid():N}.xml");
        try
        {
