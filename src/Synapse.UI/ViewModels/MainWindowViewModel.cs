...
using Synapse.Core.Features.SoftwareApps.Interfaces;
...
public partial class MainWindowViewModel : ObservableObject, IDisposable
{
    ...
    public Mode CurrentMode => _applicationModeService.CurrentMode;
    public bool IsNormalMode => CurrentMode == Mode.Normal;
    public bool IsBuilderModeActive => CurrentMode == Mode.Builder;
    public bool IsConfigReviewModeActive => CurrentMode == Mode.ConfigReview;
        ...
        IUserPreferencesService userPreferencesService)
    {
        ...
        AppIconSource = "ms-appx:///Assets/AppIcons/Synapse-rocket-white-transparent-bg.png";
        VersionInfo = "";
    ...
    public string AppTitle =>
        _localizationService.GetString("App_Title") ?? "Synapse";

    public string AppSubtitle =>
        _localizationService.GetString("App_By") ?? "by MaTows";

    // Mode switcher label + per-mode labels and tooltips
    public string ModeLabel => _localizationService.GetString("Mode_Switcher_Label") ?? " Mode";
    ...
    public string DonateTooltip =>
        _localizationService.GetString("Menu_Support") ?? "Support ";
    ...
    /// </summary>
    public async Task RequestSwitchModeAsync(Mode target)
    {
        ...
        bool leavingBuilderWithEdits = _applicationModeService.CurrentMode == Mode.Builder
            && _applicationModeService.GetBuilderEdits().Count > 0;
        bool leavingReview = _applicationModeService.CurrentMode == Mode.ConfigReview;
        ...
        if (target == Mode.Builder && !await ShowBuilderIntroIfNeededAsync())
        {
            RaiseModeProperties();
            return;
        }
        if (target == Mode.ConfigReview && !await ShowConfigReviewIntroIfNeededAsync())
        {
            RaiseModeProperties();
            return;
        }
        ...
        try
        {
            ...
            {
                case Mode.Normal:
                    if (_applicationModeService.CurrentMode == Mode.ConfigReview)
    ...
    public void UpdateAppIconForTheme()
    {
        ...
        AppIconSource = effectiveTheme == ElementTheme.Dark
            ? "ms-appx:///Assets/AppIcons/Synapse-rocket-white-transparent-bg.png"
            : "ms-appx:///Assets/AppIcons/Synapse-rocket-black-transparent-bg.png";
    }
