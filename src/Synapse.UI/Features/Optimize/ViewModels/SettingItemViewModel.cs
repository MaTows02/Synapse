...
using Synapse.UI.Features.Common.Utilities;
...
public partial class SettingItemViewModel : BaseViewModel
{
    ...
    private int? FindOptionIndex(Func<Synapse.Core.Features.Common.Models.ComboBoxOption, bool> predicate)
    {
    ...
    private bool IsBuilderMode => _applicationModeService?.CurrentMode == Mode.Builder;
    ...
    private static int FindIndexForPowerCfgValue(IReadOnlyList<Synapse.Core.Features.Common.Models.ComboBoxOption> options, int targetValue)
    {
    ...
    private (string Label, string Tooltip) ResolvePillStrings(SettingBadgeKind kind, SettingBadgeMode mode = SettingBadgeMode.None)
    {
        var (baseLabel, tooltip) = kind switch
        {
            SettingBadgeKind.Recommended => (
                ...
                _localizationService?.GetString("InfoBadge_Recommended_Tooltip") ?? "'s recommended value"),
