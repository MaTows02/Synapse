...
public class ConfigReviewService : IConfigReviewService, IConfigReviewModeService, IConfigReviewDiffService, IConfigReviewBadgeService, IApplicationModeService, IDisposable
{
    ...
    public bool IsInReviewMode => CurrentMode == Mode.ConfigReview;
    ...
    public void ExitReviewMode() => SetMode(Mode.Normal);

    public void EnterBuilderMode(BuilderTarget target) => SetMode(Mode.Builder, target);
    ...
    /// </summary>
    private void SetMode(Mode target, BuilderTarget builderTarget = BuilderTarget.Config)
    {
        LeaveCurrentMode();

        if (target == Mode.Builder)
        {
            CurrentBuilderTarget = builderTarget;
        }

        CurrentMode = target;
        _logService.Log(LogLevel.Info, target == Mode.Builder
    ...
    {
        if (CurrentMode != Mode.Builder || CurrentBuilderTarget == target)
        {
            return;
        }
        ...
        switch (settingDef.InputType)
        {
            ...
            case InputType.Selection:
            {
                ...
                if (configItem.PowerPlanGuid != null)
                {
                    ...
                    // This handles the case whereSynapse Power Plan was created with a different GUID
    ...
    private static PredefinedPowerPlan? ResolveToPredefinedPlan(string? guid, string? name)
    {
        ...
        {
            // Synapse Power Plan: match the stable brand name if a runtime GUID changed.
            if (name.Contains("Synapse", StringComparison.OrdinalIgnoreCase))
            {
                return plans.FirstOrDefault(p =>
                    p.Name.Contains("Synapse", StringComparison.OrdinalIgnoreCase));
            }
        }
