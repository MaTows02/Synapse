...
using Synapse.Core.Features.Common.Localization;
...
public class SettingLocalizationService : ISettingLocalizationService
{
    ...
    private IReadOnlyList<Synapse.Core.Features.Common.Models.ComboBoxOption> LocalizeComboBoxOptions(SettingDefinition setting)
    {
        var originalOptions = setting.ComboBox?.Options;
        if (originalOptions == null || originalOptions.Count == 0)
            return Array.Empty<Synapse.Core.Features.Common.Models.ComboBoxOption>();

        var localized = new List<Synapse.Core.Features.Common.Models.ComboBoxOption>(originalOptions.Count);
