...
namespace Synapse.UI.Features.Common.ViewModels;
    ...
    where TSectionInfo : ISectionInfo
{
    ...
    public async Task InitializeAsync()
    {
        ...
        {
            IsLoading = true;
            _logService.Log(Synapse.Core.Features.Common.Enums.LogLevel.Info, $"{LogPrefix}: Starting initialization");
            ...
            foreach (var vm in _featureViewModels)
            {
                ...
                {
                    _logService.Log(Synapse.Core.Features.Common.Enums.LogLevel.Error,
            ...
            _logService.Log(Synapse.Core.Features.Common.Enums.LogLevel.Info, $"{LogPrefix}: Loaded settings - {counts}");
            _logService.Log(Synapse.Core.Features.Common.Enums.LogLevel.Info, $"{LogPrefix}: Initialization complete");
        }
        catch (Exception ex)
        {
            _logService.Log(Synapse.Core.Features.Common.Enums.LogLevel.Error, $"{LogPrefix}: Initialization failed - {ex.Message}");
        }
        finally
        {
            IsLoading = false;
