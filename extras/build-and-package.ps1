...
# Script to automate the build and installer creation process for Synapse
#
# SYNOPSIS:
# This script builds the WinUI3 Synapse application using MSBuild and creates
...
$projectPath = "$solutionDir\src\Synapse.UI\Synapse.UI.csproj"
...
# %LOCALAPPDATA% via the Synapse_LOCAL_BUILD_ROOT env var, which both
# src\Directory.Build.props and tests\Directory.Build.props read. The env
# var is process-wide, so the child run-synapse-tests.ps1 inherits it too.
...
if ($repoIsRemote) {
    $localBuildRoot = Join-Path $env:LOCALAPPDATA 'Synapse-dev\build'
    $null = New-Item -ItemType Directory -Path $localBuildRoot -Force
    $env:Synapse_LOCAL_BUILD_ROOT = $localBuildRoot
...
# so Synapse.UI's publish output lands under <root>\Synapse.UI\bin\...
# instead of in-tree. Match whichever location the build actually used.
if ($repoIsRemote) {
    $publishOutputPath = "$localBuildRoot\Synapse.UI\bin\x64\Release\$tfm\win-x64"
}
else {
    $publishOutputPath = "$solutionDir\src\Synapse.UI\bin\x64\Release\$tfm\win-x64"
}
$innoSetupScript = "$scriptRoot\Synapse.Installer.iss"
$tempInnoScript = "$env:TEMP\Synapse.Installer.temp.iss"
...
# `dotnet test` (invoked later by run-synapse-tests.ps1) consumes stdin via
...
try {
    ...
    if ($dotnetVersion) {
        ...
        $releaseInfo = Invoke-RestMethod -Uri "https://api.github.com/repos/dotnet/sdk/releases" -Headers @{ 'User-Agent' = 'Synapse-Build' } -ErrorAction Stop |
...
try {
    ...
    $csprojXml = [xml](Get-Content -Path "$solutionDir\src\Synapse.UI\Synapse.UI.csproj")
    ...
    if ($wasdk) {
        ...
        if ($stableVersions -and [version]$stableVersions -gt [version]$currentWasdkVersion) {
            ...
            Write-Host "  Update the PackageReference in Synapse.UI.csproj" -ForegroundColor Yellow
        } else {
...
Write-Host "Building Synapse v$Version..." -ForegroundColor Cyan
...
$csprojPath = "$solutionDir\src\Synapse.UI\Synapse.UI.csproj"
...
if (-not $SkipTests) {
    ...
    & "$scriptRoot\run-synapse-tests.ps1"
    if ($LASTEXITCODE -ne 0) {
...
$mainExecutable = "$publishOutputPath\Synapse.exe"
...
$innoContent = $innoContent -replace 'AppVerName=Synapse', "AppVerName=Synapse v$displayVersion"

# Change installer output filename for beta builds
if ($Beta) {
    $innoContent = $innoContent -replace 'OutputBaseFilename=Synapse\.Installer', 'OutputBaseFilename=Synapse.Installer.Beta'
}
...
$iconPath = "$solutionDir\src\Synapse.UI\Assets\AppIcons\Synapse-rocket.ico".Replace("\", "\\")

if (Test-Path "$solutionDir\LICENSE.txt") {
    $innoContent = $innoContent -replace '; Ajoutez LicenseFile=\.\.\\LICENSE\.txt.*', "LicenseFile=$licensePath"
}
$innoContent = $innoContent -replace 'OutputDir=\.\.\\installer-output', "OutputDir=$outputPath"
$innoContent = $innoContent -replace 'SetupIconFile=\.\.\\src\\Synapse\.UI\\Assets\\AppIcons\\Synapse-rocket\.ico', "SetupIconFile=$iconPath"
$innoContent = $innoContent -replace 'Source: "\.\.\\src\\Synapse\.UI\\bin\\x64\\Release\\net10\.0-windows10\.0\.19041\.0\\win-x64\\', "Source: `"$publishPath\\"
$innoContent = $innoContent -replace 'Source: "portable\.marker"', "Source: `"$scriptRoot\\portable.marker`""
...
if ($shouldSign -and $certificate) {
    ...
    $signBatPath = "$env:TEMP\Synapse-sign.bat"
...
$installerFilename = if ($Beta) { "Synapse.Installer.Beta.exe" } else { "Synapse.Installer.exe" }
