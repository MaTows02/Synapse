...
$OutputDir = Join-Path $PSScriptRoot "..\src\Synapse.Infrastructure\Features\SoftwareApps\Services\WinGet\winget-cli"
$TempDir   = Join-Path ([System.IO.Path]::GetTempPath()) "Synapse-winget-update"
...
function Get-ReleaseInfo {
    ...
    $response = Invoke-RestMethod -Uri $url -Headers @{ 'User-Agent' = 'Synapse-Build' }
    return $response
}
