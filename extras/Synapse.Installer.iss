...
AppId={{4158A93C-FA56-47CD-99CE-9F04157D0BCA}
AppName={#MyAppName}
AppVersion={#MyAppVersion}
AppVerName=Synapse
SetupMutex=SynapseSetupMutex,Global
...
; Ajoutez LicenseFile=..\LICENSE.txt après avoir placé la licence autorisée à la racine.
...
begin
  ...
  // Append \Synapse if the path doesn't already end with it.
  //
  ...
  //     ({autopf}\Synapse for regular, {userdesktop}\Synapse for portable),
  //     silently relocating the update.
  //   - Synapse.ps1 first-time install (silent, no /DIR=): keep the sync. The
  //     PS1 relies on it to redirect a portable install from DefaultDirName
  //     ({autopf}\Synapse) to {userdesktop}\Synapse.
...
;    self-contained build. Synapse calls no ML/AI APIs, so they are dead weight -
