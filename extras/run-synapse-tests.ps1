# run-synapse-tests.ps1
# Runs all Synapse test suites and reports results.
#
# SYNOPSIS:
...
# .\run-synapse-tests.ps1 -SkipUITests
#
# # Run only integration tests
# .\run-synapse-tests.ps1 -IntegrationOnly
param (
...
Write-Host "  Synapse Test Runner" -ForegroundColor Cyan
Write-Host ("=" * 60) -ForegroundColor Cyan
...
if (-not $IntegrationOnly) {
    ...
    Run-TestProject `
        ...
        -ProjectPath "$solutionDir\tests\Synapse.Core.Tests\Synapse.Core.Tests.csproj"

    # Unit Tests - Infrastructure
    Run-TestProject `
        -Name "Infrastructure Unit Tests" `
        -ProjectPath "$solutionDir\tests\Synapse.Infrastructure.Tests\Synapse.Infrastructure.Tests.csproj"

    # Unit Tests - UI (requires Visual Studio / WinUI SDK, built with MSBuild)
    if (-not $SkipUITests) {
        $uiProjectPath = "$solutionDir\tests\Synapse.UI.Tests\Synapse.UI.Tests.csproj"
        ...
        if (-not $msbuildPath -or -not (Test-Path $msbuildPath)) {
            ...
            if ($LASTEXITCODE -ne 0) {
                ...
                $uiTestDll = "$solutionDir\tests\Synapse.UI.Tests\bin\x64\Debug\net10.0-windows10.0.19041.0\Synapse.UI.Tests.dll"
...
Run-TestProject `
    ...
    -ProjectPath "$solutionDir\tests\Synapse.IntegrationTests\Synapse.IntegrationTests.csproj"
