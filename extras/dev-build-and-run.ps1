...
#
# Bumps the Synapse.UI csproj Version / FileVersion / AssemblyVersion to
...
# when Synapse_LOCAL_BUILD_ROOT is in effect — a populated in-tree obj/
...
$csprojPath   = Join-Path $repoRoot 'src\Synapse.UI\Synapse.UI.csproj'
$msbuild      = Join-Path ${env:ProgramFiles} 'Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe'
$buildLogDir  = Join-Path $env:LOCALAPPDATA 'Synapse-dev'
...
if ($repoIsRemote) {
    ...
    $exePath = Join-Path $localBuildRoot "Synapse.UI\bin\x64\Debug\$tfm\win-x64\Synapse.exe"
}
else {
    $exePath = Join-Path $repoRoot "src\Synapse.UI\bin\x64\Debug\$tfm\win-x64\Synapse.exe"
}
...
# (via Synapse_LOCAL_BUILD_ROOT + src\Directory.Build.props). The in-tree
...
# local path via the Synapse_LOCAL_BUILD_ROOT env var, which
...
& $msbuild `
    'src\Synapse.UI\Synapse.UI.csproj' `
